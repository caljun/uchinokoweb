...
- フロー: **プロフィール写真（任意）** → **ペット種別（犬/猫）** → **犬 or 猫の1頭目登録** → `/uchinoko?welcome=1` でウェルカムモーダル

### 2.5 新規登録からの流れ

以下は、初めてアプリを使うユーザーが「はじめる（無料）」から登録して最初の1頭を登録するまでの一連の流れです。

| # | 画面・アクション | 説明 |
|---|------------------|------|
| 1 | **ランディング** `/` | 「はじめる（無料）」タップで `/auth?mode=signup` へ。既存ユーザーは「ログインはこちら」で `/auth` へ。 |
| 2 | **新規登録** `/auth` | お名前・メール・パスワード（6文字以上）を入力。招待コード（任意）は `?ref=XXX` で事前入力可。登録成功で Firebase Auth と `owners/{uid}` 作成（`friendId` 付与）、招待コードがあれば Cloud Functions でポイント付与。 |
| 3 | **リダイレクト** → `/uchinoko` | 登録直後は `/uchinoko` に遷移。メインの `(main)` レイアウトが描画され、`OnboardingGuard` が動作する。 |
| 4 | **オンボーディング開始** → `/onboarding/profile` | 飼い主に `photoUrl` が無いため、Guard が `/onboarding/profile` へリダイレクト。STEP 1/3 のプログレス表示。 |
| 5 | **プロフィール写真** `/onboarding/profile` | 写真を選んで「次へ」→ Storage に保存し `owners` を更新して次へ。「スキップ」の場合は写真なしのまま次へ。いずれも `/onboarding/pet-type` へ。 |
| 6 | **ペット種別** `/onboarding/pet-type` | STEP 2/3。犬 or 猫を選択 → 犬なら `/onboarding/dog`、猫なら `/onboarding/cat` へ。 |
| 7 | **1頭目登録** `/onboarding/dog` または `/onboarding/cat` | 名前・誕生日・体重・性別・犬種（または猫の項目）・写真などを入力。犬の場合は性格診断・生活情報も入力し、診断結果を表示。保存で `owners/{uid}/dogs/{dogId}` 作成し、`setHasDog(true)` 後に `/uchinoko?welcome=1` へ。 |
| 8 | **ウチの子一覧 + ウェルカムモーダル** `/uchinoko?welcome=1` | 登録した子が一覧に表示され、ウェルカムモーダルが開く。**最初のミッション**「今日の一枚」（+10pt）の説明と「写真を撮る」ボタンが表示される。ユーザーは「写真を撮る」か「あとで」で閉じる。 |
| 9 | **最初のミッション投稿後**（モーダル内） | 「写真を撮る」で投稿すると、モーダルが **ミッションクリア（+10pt獲得）** → **ランキングの説明** に切り替わる。「ランキングとは？」として、ミッションでポイントが貯まり**毎週リセット**されるランキングで順位が決まること、**ウチの子を1位にしよう**という案内が表示される。続けて「**ランキングを見る**」で `/ranking` へ、「**ミッションをもっとやる**」で `/missions` へ遷移する。 |
| 10 | **以降** | ウチの子一覧・ミッション・ランキング・おすすめ・マイページなど通常利用へ。 |

**補足**

- ログイン済みで「ペットがまだいない」状態でメインに来た場合も、Guard が「プロフィール写真なし」なら `/onboarding/profile`、「写真あり＆犬0頭」なら `/onboarding/pet-type` へ飛ばす。
- `/onboarding/welcome` は実装上すぐ `/uchinoko` にリダイレクトするため、通常フローでは経由しない。
- 招待リンクで来た場合: `/auth?ref=ABC123&for=dogId` で登録すると、新規登録フォームに招待コードが入り、登録完了後に上記 3 以降の同じ流れになる。
...
| パス | 説明 |
|------|------|
| `/uchinoko` | ウチの子一覧。ペット追加・各子の詳細への入口。ウェルカム時は「今日の一枚」投稿モーダル |
| `/uchinoko/new` | 犬の新規登録（基本情報＋性格診断＋生活情報→診断結果） |
| `/uchinoko/new-cat` | 猫の新規登録 |
| `/uchinoko/[dogId]` | ウチの子詳細（タブ: 基本情報 / ギャラリー）。編集・削除・共有・フレンド招待・日記一覧 |
...
- **Stripe**: `@stripe/react-stripe-js`, `@stripe/stripe-js` を依存に含む（課金・サブスクは仕様未確認のため実装箇所で要確認）
